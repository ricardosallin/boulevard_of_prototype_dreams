...
|Pergunta|Resposta|
|--------|--------|
| 1. Sallin começou perguntando sobre os "parâmetros" e escalas de tamanho (na época o GPT ainda divulgava essa info)  | GPT: <br>- modelos muito grandes como o GPT começam na casa do trilhão. Modelos muito pequenos (milhões de parâmetros) ainda podem ser úteis em contextos muito específicos.<br>- modelos menores não conseguem "conversar livremente" em linguagem geral, mas podem analisar sentimentos em frases, classificar termos, preencher lacunas etc, em contextos específicos< |
| 2. Sallin perguntou sobre a forma física desses parâmetros: um grande "CSV" ou banco de dados? | GPT: <br>- "tensores" (matrizes multidimensionais) de parâmetros: os "pesos" (números reais) e "vieses" que representam os neurônios<br>- formatos físicos: .h5 (HDFS) ou .pt (PyTorch), compactados binariamente para performance<br>- as matrizes lá da escola tinham 2 dimensões, já os tensores podem ter muito mais<br>- num exemplo de 2 dimensões, um modelo de 1 milhão de parâmetros poderia ser uma matriz 1000 x 1000 |
| 3. Sallin explicou sua ideia: criar LLM's para ler bases de dados, uma "tabela dinâmica do Excel com anabolizantes" | GPT <br>- totalmente possível como fine-tuning de um modelo existente, embedding+RAG ou treinamento de um modelo menor do zero<br>- modelos de pelo menos 1B parâmetros poderão responder análises mais complexas; menos que isso fica limitado a traduzir linguagem SQL<br>- a quantidade de colunas da base de origem impacta o tamanho do modelo necessário, mais do que a quantidade de linhas |

Nesta altura o Sallin começou a pensar mais hipoteticamente (aka pirar na batata) em modelos muito pequenos :)

|Pergunta|Resposta|
|--------|--------|
| 4. Sallin: como seria um modelo de 1 parâmetro? Uma equação do primeiro grau? O que daria pra fazer com modelos de 10, 100, 1 mil parâmetros? | GPT <br>- dezenas de parâmetros: equações polinomiais, detectar paridade de um número<br>- milhares de parâmetros: distinguir com dificuldade números escritos à mão (base do MNIST)<br>- dezenas de milhares: classificar sentimentos em frases simples, detectar sujeito/verbo/objeto, reconhecer padrões em imagens pequenas <br>- centenas de milhares: detectar palavras-chave num texto, chatbots muito simples <br>- milhão de parâmetros: já é considerado um Tiny LLM, pode escrever frases simples, responder perguntas diretas <br>- c |
| 5. Sallin começou a entender a coisa! Redes neurais são extremamente ineficientes para tarefas determinísticas (detectar paridade pode ser feito em 1 linha no Python)! Mas conforme o modelo aumenta, surgem NOVAS CAPACIDADES não previstas | GPT: É exatamente isso! O modelo é muito ineficiente, mas em grandes escalas começa a aprender padrões complexos e se adaptar a novos dados com generalização <br>- a<br>- b<br>- c |
| 6. Sallin: e o consumo computacional? Treinamento X utilização X árvores derrubadas e gatinhos mortos | GPT <br>- o treinamento envolve bilhões de operações de multiplicação de matrizes multidimensionais (backpropagation), isso exige muitas CPUs rodando por dias ou meses, gastando muita energia<br>- já na utilização (inferência) é apenas o Forward Pass, bem menos complexo, mas ainda exige milhares de multiplicações de matrizes<br>- custo da inferência depente do tamanho do modelo, do tamanho da pergunta (qtd tokens) e o hardware utilizado (CPUs x GPUs, rodar local X em nuvem)<br>- Em Watts: busca no google 0,3 W / consulta ao GPT-3.5 ~10 W / consulta ao GPT-4 ~50W / Treinar um LLM gigante Gigawatts!<br>- data centers enormes e consumo de energia, mas estão buscando otimizar<br>- e deu a ideia de criar um modelo pequeno |
